...
module.exports = class StatusCommand extends Command {
    ...
    try{
      ...
      if (!member) 
         ...
         if(!customStatus) return message.channel.send("This person is boring and doesn't have a status..").then(msg=> {msg.delete({timeout: 10000})})
         const uiembed = new Discord.MessageEmbed() 
           .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
           if (activities.length > 0) uiembed.setDescription('\u200b' + activities.join('\n'));
           if (customStatus) uiembed.setTitle("custom status;" + '\n' + customStatus)
           .setColor("#2f3136")
