...
module.exports = class PlayCommand extends Command {
  ...
  async run(message, { query }) {
    console.log(chalk.cyan.bold(`Play was ran by:`, chalk.red.bold`${message.author.tag}`, chalk.yellow.bold('in'), chalk.red.bold`${message.guild.name}`))
    ...
    if (db.get(message.member.id) !== null) {
      ...
      if (found) {
        const embed = new MessageEmbed()
          ...
          .setFooter('Choose by commenting a number between 1 and 3. © Spartan');
  ...
  static async searchYoutube(query, message, voiceChannel) {
    ...
    const embed = new MessageEmbed()
      ...
      .setFooter('Choose a song by commenting a number between 1 and 5. © Spartan.')
